...
let product_id_range a b =
  range_inclusive (Int.of_string a) (Int.of_string b) |> Seq.map Int.to_string
...
let solve1 filepath =
  ...
  |> List.map ~f:(fun (a, b) ->
         ...
         |> Seq.map Int.of_string |> Seq.fold_left ( + ) 0)
  |> List.fold ~init:0 ~f:( + )

let solve2 filepath =
  read filepath
  |> List.map ~f:(fun (a, b) ->
         product_id_range a b |> Seq.filter is_repeated |> Seq.map Int.of_string
         |> Seq.fold_left ( + ) 0)
  |> List.fold ~init:0 ~f:( + )
